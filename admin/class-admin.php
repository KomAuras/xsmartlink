...
class Admin
{
    ...
    public function assets()
    {
        wp_enqueue_style(
            $this->plugin_slug,
            plugin_dir_url(__FILE__) . 'css/xsmartlink-admin.css',
            [],
            $this->version
        );
        wp_enqueue_script(
            $this->plugin_slug,
            plugin_dir_url(__FILE__) . 'js/xsmartlink-admin.js',
            ['jquery'],
            $this->version,
            false
        );
        wp_enqueue_script(
            $this->plugin_slug . '-uploader',
            plugin_dir_url(__FILE__) . 'js/xsmartlink-uploader.js',
            ['jquery'],
            $this->version,
            false
        );
        wp_enqueue_script(
            $this->plugin_slug . '-sidebar',
            plugin_dir_url(__FILE__) . 'js/xsmartlink-sidebar.js',
        ...
        wp_localize_script($this->plugin_slug, 'wma', array(
                ...
                'Acceptors' => __('Acceptors', $this->plugin_slug),
                'PostType' => __('Select post type', $this->plugin_slug),
                'Acceptor' => __('Acceptor', $this->plugin_slug),
                'Donor' => __('Donor', $this->plugin_slug),
                'AnchorList' => __('Anchor list', $this->plugin_slug),
            )
        );
    }
    ...
    {
        register_rest_route(
            'xsmartlink/v1', '/update-meta', array(
                'methods' => 'POST',
                'callback' => array($this, 'update_post_meta_callback'),
                'args' => array(
                    'id' => array(
    ...
    public function update_post_meta_callback($data)
    {
